...
```bash
pwsh -NoProfile -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/scripts/dispatch/psmux-dispatch.ps1" -Name "<name>" -BootstrapFile "<path-to-brief.md>" -Config "<repo>/.claude/session-plugin.json"
```

To re-dispatch into an existing warm worktree (skip the dep junction step):
```bash
pwsh -NoProfile -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/scripts/dispatch/psmux-dispatch.ps1" -Name "<name>" -Task "<desc>" -SkipDeps -Config "<repo>/.claude/session-plugin.json"
```
...
### Step 3: Health-check
```
pwsh -NoProfile -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/scripts/status/check-worktree-health.ps1" -Name "<name>" -Config "<repo>/.claude/session-plugin.json"
```
Or by hand:
```
git -C "<wt>/<name>" rev-parse --git-dir
